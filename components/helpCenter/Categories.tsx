

import { getAlldata, getNotionAccount } from "@/lib/api/ExternalApi";
import { useMutation } from "react-query";
import { useEffect, useState } from "react";
import Link from "next/link";
import { useRouter } from "next/router";
import { useSession } from "next-auth/react";

export default function Categories() {
    const [categories, setCategories] = useState([]);
    const router = useRouter();
    const { data: session } = useSession();

    const { mutate: getAllPage } = useMutation({
        mutationKey: 'allpagedata',
        mutationFn: (pageId: any) => getAlldata(pageId),
        onSuccess: (res) => {
            setCategories(res.data);
        },
        onError: () => {
            // router.reload();
        }
    });
    
    const { mutate: getNotionAccountCred } = useMutation({
        mutationKey: 'notionaccount',
        mutationFn: (idAccount: string) => getNotionAccount(idAccount),
        onSuccess: (res) => {
            localStorage.setItem('token', res.access_token);
            localStorage.setItem('templateId', res.template_id);
            getAllPage(res.template_id)
        },
        onError: (error: any) => {
            session?.user?.id !== undefined && alert(error.response.data.message ?? error.response.data);
        }
    });

    useEffect(() => {
        getNotionAccountCred(session?.user?.id);
    }, [session]);
  
    return (
        <>
            <section className="py-10 bg-white sm:py-16 lg:py-30">
                <div className="px-4 py-20 mx-auto sm:px-6 lg:px-8 max-w-7xl">
                    <div className="text-center py-10">
                        <h2 className="text-2xl font-bold text-gray-900 sm:text-3xl">Choose a topic below or simply get started</h2>
                    </div>
            
                    {categories.length === 0 && (
                        <div className="text-center">
                            <div role="status">
                                <svg aria-hidden="true" className="inline w-20 h-20 text-gray-200 animate-spin dark:text-gray-400 fill-gray-200" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                                    <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                                </svg>
                                <span className="sr-only">Loading...</span>
                            </div>
                        </div>
                    )}
                    <div className="grid grid-cols-1 gap-5 mt-8 sm:mt-12 sm:grid-cols-2 xl:grid-cols-3 sm:gap-8 xl:gap-12">
                        {Array.isArray(categories) && categories?.map((item, index) => (
                            <Link
                                key={index}
                                href={{
                                    pathname: `/help-center/${item.id}`,
                                    query: {title: item.title },
                                }}
                                className="relative overflow-hidden transition-all duration-200 bg-gray-100 rounded-xl hover:bg-gray-200"
                            >
                                <div className="p-6 lg:px-10 lg:py-8">
                                    <div className="flex items-center justify-start space-x-8">
                                        <div className="flex-shrink-0 w-px h-12 bg-gray-200"></div>
                                        <div>
                                            <h3 className="text-sm font-bold text-gray-900 sm:text-base lg:text-lg">
                                                {item.title}
                                            </h3>
                                            <p className="mt-2 text-sm font-medium flex gap-x-2 text-gray-500">{item.count} <span>Available Articles</span></p>
                                        </div>
                                    </div>
                                </div>
                            </Link>
                        ))}
                    </div>
                </div>
            </section>
        </>
    );
}
